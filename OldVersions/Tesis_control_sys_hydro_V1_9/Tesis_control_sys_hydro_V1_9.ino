/*  Versión 1.9
    ...
    Comentarios: 
    - Adicion de funcion para escaneo de redes WiFi disponibles. Se establecio la red
      local como red predeterminada en la lista para intento de conexion.
    - Apagado de luces automaticamente si el sistema se encuentra activo despues de la 
      hora establecida como hora de anochecer. Se inicializo la bandera de hora del dia en 0 en vez de 1.
    - Evaluacion de estado de luces de crecimiento antes de actualizar su color, para evitar
      parpadeo de las luces durante su funcionamiento.
    - Eliminar codigo que reinicia sistema si no se ha conectado a WiFi.
    - Agregar condiciones de reinicio de bandera "network_timeout" para cuando se logra establecer conexion
      WiFi o cuando se desea intentar nuevamente la conexion.
*/
...
#define FAN2PIN 14                                                      //    ||    ||            ||        ||
#define NEOPIXELPIN 27                                                  // Pin de control de luces neopixel (cambiar a pines 18 y 19 para control individual de cada nivel de luces)
#define NEOPIXELPIN2 18                                                 //    ||                ||          
#define SERVO_PH_UP 32                                                  // Pin para control de servomotor para aumentar pH
#define SERVO_PH_DOWN 33                                                // 
...
#define OPEN 75                                                                   // Valor de grados para abrir valvula de control de pH
...
void loop()
{
  ...
  int wre_period = 30;                                                            // Attempt wifi recconect every 15 minutes
...
int scanWiFi()
{
  ...
  for (int i = 0; i < n; i++)
  {
    if(WiFi.SSID(i).c_str() == SSID_1)
    {
      return 0;
    }
    else if (WiFi.SSID(i).c_str() == SSID_2)
    {
      return 1;
    }
    else if (WiFi.SSID(i).c_str() == SSID_3)
    {
      return 2;
    }
    else
...
void initWifi() // Conexion a red WiFi utilizando credenciales PEAP no de empresa
{
  ...
  {
    case 0:
      WiFi.begin(SSID_1, password_1); // Para conexion a red de dispositivo movil
      break;
    case 1:
      WiFi.begin(SSID_2, password_1); // Para conexion a red de dispositivo movil
      break;
    case 2:
      WiFi.begin(SSID_3, password_2); // Para conexion a red local
      break;
    default:
      WiFi.begin(SSID_3, password_2); // Para conexion a red local
      break;
    ...
    if (active_time <= (growLightTime*60))
    {
      ...
      if(neo_pixel_state == 0)
      {
        ...
        for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {           // for each pixel
          ...
          NeoPixel.setPixelColor((3*pixel)-2, NeoPixel.Color(250, 255, 0));  // Amarilo | Fusia = 255, 110, 255
...
void getPhValue()
{
  ...
  ph_val = ph.readPH(voltage, w_temp_s1);
}
