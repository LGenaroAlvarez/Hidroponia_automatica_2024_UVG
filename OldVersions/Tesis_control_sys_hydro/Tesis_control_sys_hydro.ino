...
//--------------Inicializacion de sensores y luces----------------
...
//void getCurrentTime();                                                // Lectura de tiempo real
...
void loop()
{
  ...
  if (srt.start_time - srt_prev_time >= srt.time_period)
  {
    ...
    if (first_reading == true)
    {
      //Serial.println("First reading completed!");
      first_reading = false;
    }
  }
  ...
  if (first_reading == false)
  {
    ...
    //Serial.println("Checking Ambient Parameters...");
...
void initWifi() // Conexion a red WiFi utilizando credenciales PEAP no de empresa
{
  ...
  {
    network_error_flag = 0;
    //Serial.printf("Conectado a %s con direccion IP: ", SSID);
  ...
  else if(WiFi.status() != WL_CONNECTED && network_timeout >= timeout_period)// If couldn't connect and exceeded connection timeout, enable network error flag
  {
    ...
    //Serial.printf("Error connectando con la red '%s'", SSID);
    //Serial.println("Connection Timeout - Next retry in 5 minutes");
  }
}
...
void httpPostRequest(int sel)
{
  ...
  if (timer_http.start_time - http_prev_time >= timer_http.time_period)
  {
    ...
    if (status_code >= 0)
    {
      //Serial.print("POST successfull with status code: ");
...
void getCurrentTime()
{
  ...
  while (time_timeout < 3)
  {
    ...
    {
      Serial.println(&time_info, "%B %d %Y %H:%M:%S");
    }
  }
  if (time_timeout >= 2)
  {
    Serial.println("Failed to obtain current local time");
  }
}
...
void ambientAirParamsHandler()
{
  ...
  //Serial.println(setup_temp);
  //Serial.println(setup_hum);
  ...
  {
    //Serial.println("Nivel bajo de humedad!");
  ...
  else if (setup_temp > ambient_T_max || setup_hum > ambient_H_max)
  {
    ...
    fan_activate = 1;
    //Serial.println("Nivel de temperatura o humedada alta!");
  }
  else
  {
    fan_activate = 0;
...
{
  sensors.begin();
  if (!sensors.getAddress(tankSensor, 0)) Serial.println("Unable to find address for Device 0");
  if (!sensors.getAddress(growChannel1, 1)) Serial.println("Unable to find address for Device 1");
  if (!sensors.getAddress(growChannel2, 2)) Serial.println("Unable to find address for Device 2");
